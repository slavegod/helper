...
function FindTableOccurrences(Haystack, Needle)
	local Positions = {};
...
function Slice(Table, Start, End)
...
function ConcatTable(TargetTable, ...)
	local SourceTables = { ... }
...
function Serialization.SerializeModel(Items)
	...
	for Index, Item in pairs(Items) do
		...
		if Item:IsA 'BasePart' then
			...
			Datum[5] = Item.Size.Y;
			Datum[6] = Item.Size.Z;
			ConcatTable(Datum, { Item.CFrame:components() });
...
function Serialization.InflateBuildData(Data)
	...
	for Index, Datum in ipairs(Data.Items) do
		...
		then
			local Item = Instance.new(FindTableOccurrence(Types, Datum[1]));
			Item.Size = Vector3.new(unpack(Slice(Datum, 4, 6)));
			Item.CFrame = CFrame.new(unpack(Slice(Datum, 7, 18)));
		...
		if Datum[1] == Types.SpecialMesh then
			...
			Item.Offset = Vector3.new(unpack(Slice(Datum, 7, 9)));
			Item.Scale = Vector3.new(unpack(Slice(Datum, 10, 12)));
			Item.VertexColor = Vector3.new(unpack(Slice(Datum, 13, 15)));
		...
		then
			...
			Item.Color = Color3.new(unpack(Slice(Datum, 5, 7)));
		...
		if Datum[1] == Types.Fire then
			...
			Item.Enabled = Datum[4] == 1;
			Item.Color = Color3.new(unpack(Slice(Datum, 5, 7)));
			Item.SecondaryColor = Color3.new(unpack(Slice(Datum, 8, 10)));
			Item.Heat = Datum[11];
			Item.Size = Datum[12];
		...
		if Datum[1] == Types.Sparkles then
			...
			Item.Enabled = Datum[4] == 1;
			Item.SparkleColor = Color3.new(unpack(Slice(Datum, 5, 7)));
